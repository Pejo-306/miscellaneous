...
int main() {
    ...
    char opt;
    char *filename;
    printf("Please enter the file you wish to edit.\n");
    filename = get_filename();
    system("clear");

    short int i;
    do {
        printf("Currently working with:\n\"%s\"\n\n", filename);
        ...
        switch(opt) {
            ...
            case 'c':
                ...
                char *new_filename = get_filename();
                if(!create_file(new_filename)) {
            ...
            case 'r':
                ...
                filename = get_filename();
                if(!delete_file(filename)) { // returns 0 if file has been deleted successfully
		    strcpy(filename, "");
                }
                break;
            case 'e':
		...
		edit_pizza(filename, 'e');
                break;
            case 's':
                printf("Please, enter the name of the file you wish to work with:\n");
                filename = get_filename();
                break;
            case 'q':
                printf("Goodbye!\n");
    ...
    if(file) {
        ...
        while(!done) {
            ...
            if(opt == '\033') { // ESC ASCII value
                ...
                char opt, arr;
                short int i, cursor = 0, num_pizzas;
                do {
                    system("clear");
                    print_pizza(&pizza);
			...
			case 'r':
                            ...
                            // shrink file size
                            ftruncate(fileno(file), sizeof(pizza) * (num_pizzas-1));
                            printf("Pizza has been successfully deleted.\n");
                            break;
                        default:
                            printf("Please enter a valid option (in brackets).\n");
                            break;
